...
export const Tendone = () => {
    ...
    useEffect(() => {
        const fetchWeather = async () => {
            try {
                ...
                // 1. FIX: Access 'current' and use the exact key 'temperature_2m'
                const tempNum = Math.round(data.current.temperature_2m);

                // 2. FIX: Use 'weather_code' (with an underscore)
    ...
    return (
        <header className="relative w-full z-50"> {/* Higher z-index to cast shadow over content */}
            <div
            ...
            >
                <h1 className="text-3xl text-bianco text-shadow-black text-shadow-2xs italic">piùZuppa</h1>
                ...
                bg-[#3d2b1f] p-1.5 rounded-sm border-b-4 border-r-4 border-[#2a1d15] shadow-2xl z-10">
                    ...
                    <div className="bg-black px-4 py-1 border-2 border-stone-800 rounded-sm flex items-center gap-4">
                        ...
                        <div className="flex flex-col items-center">
                            ...
                            <span className="text-[9px] uppercase font-mono text-amber-700 font-bold tracking-widest mt-1">
